...
import { card , title, time, basketContainer, addedBasketContainer,  shadow, imgWrapper } from '../style/components/itemList.css';
        ...
        <div className={ card } > 
            ...
            <div>
                ...
                <div className ={inBasket?addedBasketContainer:basketContainer }>
                    <div className = { basket } onClick = {()=> inBasket?removeBasket(item.id):addToBasket(item.id) } ></div>
                </div> 
            </div>   
        </div>
    )
...
const mapStateToProps= (state , { item })=>{
    return {
        inBasket : (state.addedRecipes.findIndex((e)=>{
            return e.id === item.id
        }))>-1 ? true: false
    }
...
export default connect(mapStateToProps, { addToBasket, removeBasket })(RecipeItem);